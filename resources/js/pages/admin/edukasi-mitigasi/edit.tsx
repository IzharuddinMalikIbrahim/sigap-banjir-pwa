...
export default function EdukasiMitigasiEdit({ categories, content }: Props) {
    // Format string YYYY-MM-DD HH:mm:ss ke YYYY-MM-DDTHH:mm untuk input datetime-local
    ...
    const { data, setData, post, processing, errors } = useForm({
        _method: 'put', // Penting: Laravel butuh ini untuk update form yang mengandung file
        title: content.title || '',
        slug: content.slug || '',
    ...
    const [imagePreview, setImagePreview] = useState<string | null>(
        content.thumbnail ? `/storage/${content.thumbnail}` : null
    );

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        // Gunakan fungsi post meskipun ini proses update, karena kita upload file (thumbnail)
        post(`/admin/edukasi-mitigasi/${content.id}`);
    };
    ...
    return (
        <div className="min-h-full w-full bg-slate-50 p-4 font-sans text-slate-800 antialiased md:p-6 lg:p-8">
            <Head title={`Edit: ${content.title} - SIGAP BANJIR`} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Perbarui artikel atau panduan video yang sudah ada.
                        </p>
                    </div>
                </div>
...
EdukasiMitigasiEdit.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
