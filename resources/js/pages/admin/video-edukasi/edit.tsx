...
export default function VideoEdukasiEdit({ video }: Props) {
    ...
    const [imagePreview, setImagePreview] = useState<string | null>(
        video.thumbnail ? `/storage/${video.thumbnail}` : null
    );
    const [videoFileName, setVideoFileName] = useState<string | null>(null);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/admin/video-edukasi/${video.id}`);
    };
    ...
    return (
        <div className="min-h-full w-full bg-slate-50 p-4 font-sans text-slate-800 antialiased md:p-6 lg:p-8">
            <Head title={`Edit Video: ${video.title} - SIGAP BANJIR`} />
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Perbarui berkas video dan informasi panduan mitigasi banjir.
                        </p>
                    </div>
                </div>
                    ...
                    <div className="space-y-6 lg:col-span-2">
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
                            <div className="mb-4 flex items-center gap-2 border-b border-slate-100 pb-4">
                                ...
                                <h2 className="text-base font-bold text-slate-800">Berkas Video</h2>
                            </div>
                            ...
                            <div className="space-y-3">
                                {videoFileName ? (
                                    ...
                                    <div className="relative flex aspect-[21/9] cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-300 bg-slate-50 transition hover:border-teal-500 hover:bg-teal-50/20">
                                        ...
                                        <p className="text-sm font-bold text-slate-700">Ganti berkas video (Opsional)</p>
                                        <p className="mt-1 text-xs text-slate-400">Biarkan kosong jika tidak ingin mengubah video.</p>
                                    </div>
                                )}
                                {errors.video && <p className="text-xs font-medium text-rose-500">{errors.video}</p>}
                                {!videoFileName && video.video_path && (
                                    <p className="text-xs text-slate-500 flex items-center gap-1 mt-2">
                                        <Video className="w-3.5 h-3.5" /> Video saat ini: {video.video_path.split('/').pop()}
                                    </p>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="space-y-4">
                                {imagePreview ? (
                                    ...
                                    <div className="relative flex aspect-video cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 bg-slate-50 transition hover:border-teal-500 hover:bg-teal-50/50">
                                        ...
                                        <p className="text-sm font-semibold text-slate-600">Ganti gambar (Opsional)</p>
...
VideoEdukasiEdit.layout = (page: React.ReactNode) => <AdminLayout children={page} />;
